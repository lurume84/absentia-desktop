...
int RunMain(HINSTANCE hInstance, int nCmdShow) 
{
  ...
  CefString(&settings.user_agent).FromString("AbsentiaBrowser");
  ...
  
  {
	  ...
	  if (boost::filesystem::exists(applicationService.getViewerFolder() + "/index.html"))
	  {
		  ...
		  window_config.url = iniFileService.get<std::string>(applicationService.getMyDocuments() + "Absentia.ini", "FileServer", "Endpoint", "http://127.0.0.1:9292/");
	  }
	  else
	  {
  ...
  subscriber.subscribe([](const desktop::core::utils::patterns::Event& rawEvt)
  {
	  ...
	  std::stringstream ss;
	  ss << "Absentia " << evt.m_version << " is available, do you want to install it?";

	  std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
	  std::wstring message = converter.from_bytes(ss.str());

	  const int result = MessageBox(NULL, message.c_str(), L"Absentia upgrade", MB_YESNO);
